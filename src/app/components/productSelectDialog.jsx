    ...
    <>
      ...
      >
        ...
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
          ...
          <TableContainer className="glass-card" sx={{ maxHeight: 400, flexGrow: 1, overflow: 'auto', border: '1px solid var(--glass-border)' }}>
                ...
                {Object.entries(grouped).map(([name, variantsList]) => {
                  ...
                  return (
                    <TableRow
                      ...
                      sx={{
                        cursor: "pointer",
                        bgcolor: isSelected ? "rgba(0, 137, 123, 0.1)" : "transparent",
                        '& td': { border: 0 }
                      }}
                    >
                      <TableCell sx={{ fontWeight: isSelected ? 700 : 400 }}>{name}</TableCell>
                      <TableCell>{earliest.barcode}</TableCell>
                      <TableCell>
                        <Typography variant="body2" color={earliest.quantity < 5 ? "error" : "inherit"}>
                          {earliest.quantity}
                        </Typography>
                      </TableCell>
                      <TableCell sx={{ fontWeight: 600, color: 'var(--primary)' }}>{earliest.price}</TableCell>
                    </TableRow>
                  );
                })}
              </TableBody>
          ...
          {selectedProduct && (
            <Box className="glass-card" sx={{ p: 2, display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap', mt: 1, bgcolor: 'rgba(0, 137, 123, 0.05)' }}>
              <Box sx={{ flex: 1, minWidth: 150 }}>
                <Typography variant="caption" sx={{ display: 'block', mb: 0.5, color: 'var(--primary)', fontWeight: 600 }}>الكمية</Typography>
                <TextField
                  type="number"
                  fullWidth
                  value={tempQuantity}
              ...
              <Box sx={{ flex: 1, minWidth: 120 }}>
                <Typography variant="caption" sx={{ display: 'block', mb: 0.5, color: 'var(--primary)', fontWeight: 600 }}>الوحدة</Typography>
                <Select
                  fullWidth
                  value={tempUnit}
                ...
                >
                  {(selectedProduct?.unitOptions || ["علبة"]).map((u) => (
                    <MenuItem key={typeof u === "string" ? u : u.value} value={typeof u === "string" ? u : u.value}>
              ...
              {selectedProduct?._id !== "agel" && (
                <Box sx={{ flex: 1.5, minWidth: 180 }}>
                  <Typography variant="caption" sx={{ display: 'block', mb: 0.5, color: 'var(--primary)', fontWeight: 600 }}>تاريخ الصلاحية</Typography>
                  <Select
                    fullWidth
                    value={tempExpiry}
                    onChange={(e) => {
                      const newVariant = variants.find((v) => (v.expiryDate ?? "no-expiry") === e.target.value);
                      if (newVariant) {
      ...
      >
        <Alert severity="error" variant="filled" onClose={() => setError(null)} sx={{ width: '100%', borderRadius: '12px' }}>
          {error}
        </Alert>
      </Snackbar>
    </>
